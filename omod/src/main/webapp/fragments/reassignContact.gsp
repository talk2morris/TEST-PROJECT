...
<script>
    ...
    if(obj !="")
    {
        ...
        for(var i=0;i<obj.length;i++)
        {
            ...
            userID = obj[i].id+'';
            userName = obj[i].firstname+''+obj[i].lastname;
            
           
            console.log(userID);
            console.log(userName);
            
            assignButton = '<button type="Reload List" class="btn btn-primary heading-text" style="width: 80%;" onclick="showAssignmentTester(' + userID + ')">'+"Re-assign"+'</button>';
            if(assignmentNullCheck == 'null')
            {
                assignmentNullCheck = 'Unassigned';
                assignButton = '<button type="Reload List" class="btn btn-primary heading-text" style="width: 80%;" onclick="showAssignmentTester(' + userID + ')">'+"Assign"+'</button>';
            ...
            if(obj[i].trace_status == 'pending' || obj[i].trace_status == 'Pending')
            {
                ...
                jq('#AssignTableBody').append("<tr><td>"+obj[i].lastname+"</td><td>"+obj[i].firstname+"</td><td>"+assignmentNullCheck+"</td><td>"+assignButton+"</td></tr>");
            }
